...
def create_model(in_shape: int, out_shape: int):
    ...
    model.add(
        layers.Dense(264, activation="relu", kernel_regularizer=regularizers.l2(0.10))
    )  # Hidden layer 1
    model.add(layers.Dropout(rate=0.2))
    model.add(
        layers.Dense(128, activation="tanh", kernel_regularizer=regularizers.l2(0.10))
    )  # Hidden layer 2
    model.add(
        layers.Dense(64, activation="relu", kernel_regularizer=regularizers.l2(0.10))
    )  # Hidden layer 3

    # Output layer
    model.add(layers.Dense(out_shape))  # No activation (regression problem)
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
    ...
    val_mse = history.history["val_mse"][-1]  # Get the last validation MSE
...
print(f"Cross-Validation results: Mean MSE = {mean_mse}, Std MSE = {std_mse}")

# Create a new instance of the model
model = create_model(in_shape=X.shape[1], out_shape=y.shape[1])
model.fit(X, y, epochs=30, batch_size=64, verbose=1, shuffle=True)
